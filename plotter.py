...
# TO-DO: Verify things are working correctly and use pareto set of each gen for some plots.
...
class Plotter():
    ...
    def _update_interactive_convergence_scatter(self, x_data: list, y_data: list, z_data: list,
    ...
    def calc_pareto_set(self, gen_data: list):
        """
        Returns a list of non-dominated objective data points from the collection of 3-tuples. 
        """
        # ---
        # O(n^2) time comp but not horrible since usually n < 200. Replace it if time permits.
        # ---
        non_dominated = []
    ...
    def update_while_training(self):
        ...
        while current_generation < self.max_gen:
            if not self.queue.empty():
                ...
                print(len(self.pareto_gen_data[-1]))
                ...
                y_data = list(y_data)  # Unnecessary
                z_data = [-y for y in y_data]
    ...
    def create_gen_scatter(self, title: str, dimensions: int, gen: int) -> None:
        ...
        fig.savefig(f"Assets/Images/{timestamp}_{title}_scatter.png")
